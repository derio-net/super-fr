...
**Operator updates** only at these two stops, a block or failure, and delivery. Each update is the
result in 1–3 lines, then the next step. Short means split, not packed: plain words, name what an
id refers to, keep the "because". Evidence goes to the debug journal and the PR body, not the chat.
