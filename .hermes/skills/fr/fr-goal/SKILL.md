...
Verify first (`superpowers:verification-before-completion`): run the full suite YOURSELF into a log —
an executor's "green" is a claim, not evidence — and name it `--evidence tests=<log>`; self-review
...
summary + spec/plan paths; findings + fixes (+ refutations, + deferrals with their issues) and decisions from BOTH journals, `fr journal render --scope spec --section findings` and `--scope plan --section findings`/`decisions` — each renders its out-of-scope findings under their own heading: keep that section, asking the operator which to file as issues (answered at the merge touchpoint, never blocking `deliver`); confirm `git status` is clean for plan and journals (fr commits its own writes as `chore(fr):` commits; commit any hand edits yourself), then paste `fr plan proportionality <plan-dir>` verbatim (`resolve` re-derives it from HEAD as the `proportionality` witness — above 2× the estimate, add a justification line) and the `fr run cost <run-id>` table (`—` is unobservable, never zero); an **Operator gates** section
...
commits since the ok — fr's own `chore(fr):` record commits don't count against this). ONLY when all three hold: `gh pr ready`, remove the guard — never say
"ready to merge" before this, never self-merge, never flip it manually. Resolve `deliver` done
with that `tests=` log (fr checks YOU wrote it, this session, during delivery — not that it is a real suite); it prints a commit sha to `git push` (or says the commit was refused — commit and
push it yourself before merging) and a `closeout: … fr pickup --run <run-id>` handoff line —
`git push`, then relay that handoff line to the operator verbatim. Stop; the operator merges.

### Post-merge close-out
Runs in a **NEW session**, started from the base clone, not the delivering session — the
operator opens it with the `fr pickup --run <run-id>` line relayed at `deliver` (or the same
line `fr run advance` reprints for an already-finished run). That command refuses, exit 2, a
run whose `deliver` isn't done; otherwise it prints a self-contained brief — branch, PR, spec
and plan paths, and the closeout steps as exact commands, built only from the run file, so this
new session needs no memory of the one that delivered. Follow it in order: `fr isolation
verify-merge --branch <b>` (no `--default-branch` needed — it resolves the remote's own default)
— not verified → STOP and recover (cherry-pick / fresh PR); the spec's Test Plan if present; for
each out-of-scope finding the brief lists, open the issue and run its `fr journal resolve
--scope <spec|plan> --slug <s> --id <f> --state deferred --tracked-by <#N> --note "<why>"` line
(unanswered ones stay out-of-scope, visible); `fr status`; `fr isolation up --branch
chore/archive-<slug>` (never inside the just-merged feature workspace), `fr archive <plan-dir>`,
commit and push, open the housekeeping PR; `fr isolation down --branch <b>` (or let gc reap it).
