...
drive the Test Plan if present; for each out-of-scope finding the operator said to file, open the issue and `fr journal resolve --scope <s> --id <f> --state deferred --tracked-by <#N> --note "<why>"` (unanswered ones stay out-of-scope, visible); confirm phases complete (`fr status`), `fr archive
