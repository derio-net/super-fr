...
environment is repaired with `fr isolation rebuild` (worktree kept), never `down --force`;
a forced down preserves the run record, but the workspace and its uncommitted fix are gone.
