...
- **There is a THIRD mirror tree, with its own script.** `.hermes/skills/**`
  is generated by `scripts/sync-hermes.py` (same contract: no flag writes,
  `--check` verifies), guarded by `test_tripwire_hermes_skills_sync.py`. It is
  easy to miss precisely because this section used to name only the OpenCode
  script: an agent could edit a canonical skill, run `sync-opencode.py`, pass
  every check it had been told about, and still ship a stale Hermes mirror —
  caught only by running the FULL suite. Editing any canonical skill means
  running **both** sync scripts.
