...
<!-- fr:journal kind=finding scope=plan id=p4-status-claims-merged created=2026-09-20T17:20:04 phase=4 state=open -->
### p4-status-claims-merged · finding [open] · fr status labels a locally-complete plan 'merged but not archived' and recommends archiving it — with nothing merged (phase 4)

Flagged by phase 4, root-caused by me. NOT a logic bug: fr.archive.completed_unarchived_plans is deliberately gh-free (#334) and counts a plan iff every phase satisfies render.plan_locally_complete. Its own docstring puts 'merged-but-unarchived' in scare quotes, acknowledging the proxy. The defect is the USER-FACING LABEL: status_cmd prints 'archivable — merged but not archived' and then 'run fr archive --all to move 1 plan(s) to implemented/', asserting an observation the check never makes.

Why it matters more than it looks: every fr-goal run reaches this state BEFORE the PR is opened, because the pipeline completes all phases and only then delivers. So the recommendation to archive fires precisely when archiving would be wrong, and archiving is close to irreversible in this repo — docs/superpowers/implemented/ is frozen, no locator reaches it, no migration rewrites it, and fr validate artifacts does not check it (.claude/rules/artifact-versioning.md). An operator who follows the suggestion files unshipped work into the archive.

Observed here: 'archivable — merged but not archived (1): 2026-09-20-agentic-dispatch-verb-lint' with nothing merged and no PR yet open.

It is also the same defect class as #428 itself, one level up — a claim of something that was never performed. The executor ticked a step it could not carry out; fr status reports a merge it never observed. Both are a check whose output asserts more than its evidence.

Fix shape (not done here, out of scope): say what is actually known — 'complete locally, not yet observed merged' — and make the suggested command the safe one, or have the sweep consult the tracker before using the word merged. Needs its own issue.

<!-- fr:journal kind=finding scope=plan id=p4-i1-matrix-note-false created=2026-09-20T17:31:03 phase=4 state=fixed -->
### p4-i1-matrix-note-false · finding [fixed] · I1: a matrix note explained the corpus delta with the wrong cause, and it had already reached three committed reports (phase 4)

Review finding, verified. goal-dispatch-lint-precision's notes said the brainstorm's 43/1419 'was an active-roots count; the harness reads the archived roots too'. False: that figure was the same TWO roots read with enforce_fr_version=True, and the delta to 82/2113 is the version gate. Measured, the live root alone is 5 plans / 134 agentic steps — nowhere near 43. A maintainer widening the corpus would have 'fixed' root coverage and left the version gate alone, and the floors would stop meaning what they say. This is the second instance of the same hole in one phase: fr acceptance check validates --level refs and NEVER reads prose notes, so a wrong claim (or, in the earlier near-miss, a fabricated test name) in a notes field passes every gate this repo has and propagates into report_local.html, report_linked.html and report_linked.md. FIXED via fr acceptance set-status, which rewrote the row and regenerated all three reports; the corrected note now states the real cause AND says why, so the error cannot be reintroduced by someone reading only the note.

<!-- fr:journal kind=finding scope=plan id=p4-i2-cli-test-name-lied created=2026-09-20T17:31:04 phase=4 state=fixed -->
### p4-i2-cli-test-name-lied · finding [fixed] · I2: test_..._names_both_escapes asserted one escape (phase 4)

Review finding, verified by reading the assertion tuple. The CLI test asserted 'no Agent tool', 'task: deny', '[manual] phase' and '#428' — the [manual] escape but not the other one, 'Name the OUTCOME instead (...)'. Deleting the outcome-naming clause from the message left the test green, and the goal-agentic-dispatch-purity matrix note repeats the test's claim verbatim ('both escapes named'). The unasserted half is precisely the half decision d1 called the thing that makes the lint actionable: a gate that errors without saying what to write instead is half a fix. FIXED: 'Name the OUTCOME' added to the token tuple, with a comment saying why it must not be droppable in silence. A test whose NAME claims more than its body is the same defect class as a tick that claims more than the work.

<!-- fr:journal kind=finding scope=plan id=p4-i3-stale-code-citation created=2026-09-20T17:31:04 phase=4 state=fixed -->
### p4-i3-stale-code-citation · finding [fixed] · I3: this PR staled a code citation inside the very paragraph it edited (phase 4)

Review finding, verified. docs/explainers/01-fr-goal.md cited packages/fr/src/fr/plan_ops.py:867-1029 for self_review's behaviour. On main that range held ReviewIssue + self_review (def at 876); after this PR inserted ~264 lines of detector above it, self_review is at 1079-1245 and 867-1029 is the regex block — a reader following the citation to see the behaviour the paragraph describes lands on regexes. test_tripwire_explainers_fresh.py checks only title and headings, so it cannot catch a drifted line range; the rule's own 'Known gaps' section says as much. FIXED to 1079-1245, and the fr-plan/SKILL.md range corrected 63-84 → 61-83 for the same reason (phase 3 edited that file). Worth remembering: an explainer that cites line ranges is a cross-file invariant with no guard, and the PR most likely to break it is the one editing both files.

<!-- fr:journal kind=discovery scope=plan id=p4-explainer-widened created=2026-09-20T17:31:04 phase=4 -->
### p4-explainer-widened · discovery · The explainer paragraph was rewritten rather than extended, to cover the executor half and fix three prose defects (phase 4)

Review raised M4/M5/M9 together and they had one answer. M4: 'For the same reason' attached grammatically to workflow-shape resolution, not to the manual-work clause two clauses back. M5: 'the phase executor' arrived cold, 40 lines before the page first explains what one is. M9: the PR ships an executor-side behaviour change (refuse the tick, leave it unticked) that the page did not mention at all, which explainers-currency would otherwise oblige a written justification for. Rewritten as its own short paragraph: it names the leaf property, forward-references step 6 where the phase executor is introduced, states the tick-as-false-claim consequence, ties it explicitly to manual-work-in-an-agentic-phase as the same defect class, and covers the executor contract in one clause. Re-rendered from / with --isolated; the page diff is proportional to the source diff, so codehilite did not flip.

<!-- fr:journal kind=discovery scope=plan id=rebase-tiers-were-eaten created=2026-09-20T17:39:05 -->
### rebase-tiers-were-eaten · discovery · This plan's authored tiers were silently dropped by fr plan create 4.8.0 — live corroboration of gh-434

Found while rebasing onto origin/main, which had moved to 4.8.1 with #506 (gh-434, 'phases-file tier ingestion'). My phases.yaml declared tier: standard / hard / standard / standard per operator decision d4; fr plan create at 4.8.0 wrote the plan folder WITHOUT them and said nothing. The loss only became visible after the rebase, because #506 also added the self-review warning that surfaces an untiered agentic phase — four warnings, one per phase.
