...
<!-- fr:journal kind=review scope=debug id=79eda66a8914 created=2026-09-22T19:52:42 -->
### 79eda66a8914 · review · Independent review of 2209b2f2: one finding (version bump), fixed

A separately dispatched reviewer read the fix statically (it had no shell). Its one finding was that the version was not bumped, which `version-bump-required` would reject because `scripts/install.sh` and `packages/fr-opencode-plugin/src/**` changed. The bump had been deferred until #536 (4.16.0) merged. It is now **fixed**: minor, 4.17.0, because this is a new install-time delivery to consumers. The reviewer checked and cleared: the install/uninstall call placement inside the right `if`/`fi`; the marketplace rsync carrying `packages/fr-opencode-plugin/src`; the `rm -rf` guards; the no-`await` check-and-set; that the live sentinel resolves to the shipped loader file (LOAD_AS_FILE before directory); and parity notes that do not overclaim. It left one question open, not a finding: whether `opencode debug config` needs the network in CI. The CI run answers that.
