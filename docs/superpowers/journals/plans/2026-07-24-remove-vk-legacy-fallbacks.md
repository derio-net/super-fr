...
<!-- fr:journal kind=finding scope=plan id=p1-acceptance-row-not-implemented created=2026-07-24T19:32:50 phase=1 state=fixed -->
### p1-acceptance-row-not-implemented · finding [fixed] · Acceptance row vk-legacy-fallbacks-removed flipped to ci in phase 3 (phase 1)

Phase 1 completion warned the acceptance row 'vk-legacy-fallbacks-removed' was still not-implemented (matrix backfill was out of phase-1 scope; phases 2–3 remained). Resolved in phase 3: `docs/acceptance/matrix.yaml` row flipped not-implemented → ci, citing the fallback-gone unit tests in test_isolation.py (test_profiles_config_ignores_lone_vk_profiles, test_load_state_ignores_legacy_vk_dir, test_list_states_ignores_legacy_vk_dir, test_up_legacy_vk_mount_hard_errors) and test_bridge_cli.py (test_bridge_env_vk_ignored) that landed in this PR. `fr acceptance check` passes (exit 0).
