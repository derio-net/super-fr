...
Second 'fr apply --yes' against the live GitLab died: RepoLabelEnsure — 'Post .../labels: 409 {message: Label already exists}'. Because label ensure runs FIRST and the failure propagates, the whole apply aborts and the pending body updates never land, so the plan cannot converge: run 1 creates, run 2 fails, forever. fr.glab.ensure_label's own docstring already describes the correct behaviour — 'a pre-existing label name causes an error, WHICH THE CALLER (RealGlabClient.ensure_labels) TOLERATES' — and no code anywhere implements that tolerance; fr.glab.ensure_labels says 'First failure propagates (mirrors fr.gh.ensure_labels)', and gh only gets away with that because it passes --force, which glab has no equivalent of. So this is the same defect family as everything else in this PR: a docstring describing behaviour the code does not have, load-bearing this time. A SECOND lesson in the captured error text: rich wrapped it mid-phrase as 'Label already\n  exists', so a naive substring test for 'already exists' would not match — the same wrapping that makes tests/unit/test_run_workspace.py fail on this Mac and pass in CI. The fix therefore normalizes whitespace in _haystack, which also hardens is_not_found and is_transient against a wrapped 404 or timeout message they would silently miss today. IN SCOPE: the issue's closing condition 2 is the end-to-end apply demonstrated, and an apply that cannot run twice does not demonstrate it.

<!-- fr:journal kind=finding scope=plan id=f16-body-never-converges created=2026-09-19T22:03:43 phase=7 state=fixed -->
### f16-body-never-converges · finding [fixed] · FOURTH bug from the live walk: GitLab strips the rendered trailing newline, so the body diff never converged (phase 7)

After f14 and f15 were fixed, fr apply STILL wanted the identical body update on every run: 'update body on ...#1 (1270 chars)' forever, while GitLab had stored 1269. Compared byte counts directly against the API: fr renders a trailing newline, GitLab strips trailing whitespace from an Issue description, and diff.py:218 compared with an exact !=. So the plan was permanently non-convergent — the third independent reason the acceptance row's own sentence ('phases rendered, observed, diffed and applied') was false. FIXED by comparing obs.body.rstrip() != ri.body.rstrip(). Normalised in the comparison rather than in the GitLab adapter, because trailing whitespace is not a meaningful body change on any forge, and having the adapter re-append a newline GitLab will not keep would be a fiction maintained to satisfy a comparison. After the fix the walk converges: apply --yes, then a re-run reporting no Issue mutations, then a state change (ticking a step) propagating and re-converging immediately.

<!-- fr:journal kind=finding scope=plan id=f14-gitlab-work-items-url-resolved created=2026-09-19T22:05:46 phase=7 state=fixed resolves=f14-gitlab-work-items-url -->
### f14-gitlab-work-items-url-resolved · finding [fixed] · resolves f14-gitlab-work-items-url: SECOND independent bug, found by the live walk: fr cannot parse the Issue URL GitLab hands back, so observe crashes on every re-apply (phase 7)

_urls.ISSUE_URL_RE and _ISSUE_NUM_RE now accept (?:issues|work_items), and _hosts._URL_SHAPES matches /-/(?:issues|work_items)/<n>, so observe reads back the URL fr stored and fr_dispatch.prompt resolves a real GitLab tracking URL to gitlab. Pinned by tests carrying the live-captured URL verbatim, including a repo NAMED work_items to hold the anchoring lesson from f11. Live: fr apply now re-runs without crashing.

<!-- fr:journal kind=finding scope=plan id=f15-label-ensure-not-idempotent-resolved created=2026-09-19T22:05:46 phase=7 state=fixed resolves=f15-label-ensure-not-idempotent -->
### f15-label-ensure-not-idempotent-resolved · finding [fixed] · resolves f15-label-ensure-not-idempotent: THIRD independent bug from the live walk: ensure_labels aborts fr apply on a pre-existing label, so it can never converge (phase 7)

fr.glab.ensure_labels now skips an already-exists refusal via the new is_already_exists and continues, which is the behaviour ensure_label's docstring had claimed since 2026-07-09 and nothing implemented; both docstrings corrected to say where the tolerance actually lives and what it gives up (colour drift on an existing label stays uncorrected, since glab has no --force). _haystack additionally collapses whitespace, because rich had wrapped the 409 as 'Label already\n  exists' — which also hardens is_not_found and is_transient against a wrapped 404 or timeout. Live: the second apply --yes now completes instead of aborting.

<!-- fr:journal kind=discovery scope=plan id=d-four-findings-left-open-deliberately created=2026-09-19T22:06:14 phase=7 -->
### d-four-findings-left-open-deliberately · discovery · Four findings ship OPEN on purpose — fr journal resolve has no state for 'true, but out of scope' (phase 7)

fr-goal §7 wants 'fr journal check --scope plan' clean before deliver, and fr journal resolve --state accepts only fixed | refuted. Four findings here are neither: they are TRUE and deliberately not fixed by this PR. Refuting them would mean recording that a correct finding was wrong, to make a gate go green — which is precisely the behaviour this whole PR is a case study against, so it is refused. They ship open, each named in the PR body with its reason and its follow-up. (1) ca6cb2838610 — three environment-dependent pre-existing test failures on macOS (a long tmp_path pushing rich's wrap point into an asserted phrase; a real super-fr marketplace on the machine defeating a test-isolation assumption). Proven unrelated: they predate the branch and pass inside the Linux container. Owed its own issue. (2) f-reachability-traceback — fr_dispatch.reachability must render a propagating GlabError as a refusal rather than a traceback; it has NO production caller today (apply_cmd passes no gh=, check_reachable is test-only), so nothing regresses and there is no caller to fix. An obligation on whoever completes that cutover; the spec's risk table §5 is its durable home. (3)+(4) 4d9ab22d64fe and 4d9ab22d64fe-arity — one gap tracked twice, which is itself worth noting: pr_state's Issue auto-close has the right backend and the wrong host, blocked on the public closer: Callable[[str, str, str], None] arity that every test double satisfies structurally. Widening it is bridge-wide work beyond gh-486. THE TOOLING GAP IS REAL AND WORTH CLOSING: a journal needs a 'deferred' or 'wont-fix-here' state, or fr journal check needs to distinguish 'unresolved' from 'unacknowledged'. As it stands the gate's only clean path is to mislabel, which is a gate teaching the wrong lesson.

<!-- fr:journal kind=finding scope=plan id=f17-bare-409-silent-skip created=2026-09-19T22:18:56 phase=7 state=fixed -->
### f17-bare-409-silent-skip · finding [fixed] · is_already_exists matched a bare 409, and a false positive there is SILENT — a label never created, no error (phase 7)

Phase 7's review found the one real soft spot in the in-phase fixes: _ALREADY_EXISTS_PATTERNS contained a bare ': 409 ', unscoped to label conflicts. The reviewer could not construct a realistic false positive (the predicate is consulted only from ensure_labels, which only ever calls 'glab label create', where GitLab's 409 vocabulary is effectively single-purpose) but named the thing that makes it matter anyway: the failure mode is ASYMMETRIC. A match makes ensure_labels skip silently, so a false positive is a label that was never created with no error to say so — strictly LESS discoverable than the loud abort it replaced. FIXED by requiring either the phrase 'already exists' (reachable only because _haystack collapses rich's mid-phrase wrap) or a 409 PAIRED with the API's '{message:' envelope, so a stray gateway or proxy 409 no longer reads as 'already there'. Verified: the live captured 409 still matches, 'proxy rejected the request : 409 ' and 'Error: HTTP 409' no longer do. Also added a DEBUG log on each skip rather than a warning — on any re-apply every label already exists, so warning would print once per label on every run and train the operator to ignore it; DEBUG leaves a trail without the noise. And copied is_transient's self-aware 'this widens the input surface' caveat onto is_already_exists and is_not_found, which now share the identical mechanism (review Minor #3).
