...
<!-- fr:journal kind=discovery scope=plan id=x-p5-opencode-walk created=2026-09-26T13:17:16 phase=5 -->
### x-p5-opencode-walk · discovery · Phase 5 OpenCode walk (devcontainer) passes: run 2026-09-26-fix-659-isolation-gc-fetch, PR #679 (phase 5)

After the lean-cost-aware-process host-side rule (fr run executes on the harness host, never inside fr isolation exec): usage/2026-09-26-fix-659-isolation-gc-fetch.yaml holds an opencode capture in devcontainer mode with the bound session and per-step figures for all 7 steps; every cursor attempt records harness=opencode (the #605 walk recorded None); spec-review answered by a child session running fr-spec-reviewer-standard (marked unobserved, as OpenCode's parity row states); PR #679 carries the out-of-scope, proportionality and cost sections, now enforced by deliver. Session found via an explicit binding — automatic OpenCode session discovery is #636.
