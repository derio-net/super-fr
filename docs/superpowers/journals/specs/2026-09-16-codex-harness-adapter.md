...
<!-- fr:journal kind=decision scope=spec id=d12 created=2026-09-16T08:22:22 -->
### d12 · decision · d12: fr codex install defaults to dry-run; writing needs an explicit flag

Operator decision 2026-09-16, amending d3/d5. The user-level install shape is approved as designed: a delimited managed block in ~/.codex/AGENTS.md, full rule bodies under $CODEX_HOME/rules/super-fr/, and merged entries in ~/.codex/hooks.json with absolute command paths (the fr hermes install pattern). The amendment: the default invocation must SHOW the diff and write nothing; an explicit flag performs the write. Reason: these are the operator's global, cross-project Codex files, and this session already had to repoint eleven dangling links there by hand; a preview-by-default install makes every global mutation inspectable before it happens, at the cost of one extra flag. uninstall keeps its existing reversal semantics. Phase 5 owns this, and its tests must assert that a bare invocation leaves all three targets byte-identical.

<!-- fr:journal kind=decision scope=spec id=d13 created=2026-09-16T08:22:23 -->
### d13 · decision · d13: ship the design as a draft PR; implementation phases wait for the operator

Operator decision 2026-09-16. The dispatched agent completed brainstorm, spec, spec-review, plan and plan-review, then handed back mid-phase-1 with nothing implemented and nothing pushed. Rather than resume six phases unattended, the operator chose to publish the design for review first: push feat/codex-harness-adapter and open a DRAFT PR carrying the spec, plan, journals and the five not-implemented acceptance rows. Consequence to keep honest: willikins#405 continues to state that Codex has no fr enforcement hook, because none exists. Resume path is unchanged: fr run advance 2026-09-16-feat-codex-harness-adapter, then serial fr-phase-executor dispatches, never with isolation worktree.
