...
<!-- fr:journal kind=review scope=spec id=r-spec1 created=2026-09-20T08:52:57 -->
### r-spec1 · review · Spec-review: two findings, both fixed in the spec

Every claim checked against the code. `fr/opencode_agents.py` does not exist (new module, correct); `fr.models.default_models_path` does honour $XDG_CONFIG_HOME then $HOME/.config, so the "same resolution" claim holds; `models_cmd.set_cmd` does call `set_binding` then print, so appending the materialiser call is exactly as described.

r1 — §3.A hardcoded the target as `fr-phase-executor-<tier>.md`. Wrong: the materialiser cannot read `plugins/super-fr/agents/` (no checkout is guaranteed when `fr models set` runs), so a literal stem would be a SECOND source of truth for the agent set with nothing reconciling the two — and it would silently skip a second canonical agent the moment one exists. FIXED: targets are discovered by globbing the installed dir and matching any stem ending in `-<tier>` for a tier in PHASE_TIERS. install.sh`s existing regex `^(.+)-(mechanical|standard|hard)\.md$` already works this way, so this is that rule moved rather than a new one invented.

r2 — Test Plan item 3 said the three tests from PR #495 "must pass unchanged". False for two of them, and checked rather than assumed: `test_install_runs_agents_delivery_after_fr_cli_install` anchors on the comment "# Derive the tier from the filename suffix", which lives inside the awk block this spec deletes, and `test_install_calls_fr_models_resolve_for_agents` asserts the literal string "fr models resolve --harness opencode", which becomes `fr models apply`. FIXED: the item now separates the three INTEGRATION tests (behavioural — they must pass unchanged, and are the safety net for swapping the implementation) from the two UNIT text-assertions that must be updated to assert the same property against the new call. Left unfixed, this would have surfaced mid-phase as a surprise regression and invited an executor to "fix" the tests rather than notice they encode a deleted implementation detail.
