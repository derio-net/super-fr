...
- **Phase 8 — Agent dispatch prompt: merged "BEFORE YOU BEGIN" preamble** (added 2026-05-18 after the post-Phase-4-merge stale-checkout incident)
  - The bridge pod's `~/repos/<repo>` checkouts are NOT auto-refreshed and are **shared with the operator**, so we can't have the bridge `git pull` either — that would clobber operator's in-progress work. After Phase 4 shipped, the bridge kept projecting Phase 5 as `vk-blocked` because its checkout was 9 hours stale and didn't see Phase 4's `completion.at` timestamp; operator had to manually run `git pull` in the pod to unblock dispatch. Once a workspace IS dispatched, its branch is also based on stale `main`, so the agent's work could rebase on top of already-merged code (silent regression risk) or produce conflicts at PR time.
  - **Fix is agent-side, not bridge-side**: in `src/vk/bridge/prompt.py`, replace the existing standalone `_DEPS_PREAMBLE_TEMPLATE` with a single combined preamble built from a numbered list. Item 1 fires unconditionally (sync); item 2 fires conditionally (deps). The new shape: `"BEFORE YOU BEGIN:\n1. Fetch and rebase your worktree on origin/main: ... If rebase conflicts, STOP and report.\n[2. This Issue declares dependencies: #N. Verify each is CLOSED ...]\n\n---\n\n"`. Much shorter than two separate preambles glued together — both items in one block, agents scan it as one pre-flight checklist.
  - **Why not auto-pull in the bridge instead**: violates shared-pod ownership. The pod runs operator workflows alongside the bridge daemon; an auto-pull would mid-execute clobber in-flight operator branches. Agent-side compensation is the right shape for the shape of the constraint.
  - Single-task phase. Source change is one file (`src/vk/bridge/prompt.py`) plus its existing unit tests + golden-prompt fixtures (which break because preamble shape changes). Patch version bump.
  - Depends on Phase 6 (cutover) so the new preamble flows into the post-cutover bridge surface, not the legacy one being retired.

Each phase is one PR (per the repo's "one phase = one PR" convention). Phase 6 spans both repos and is the only cross-repo phase. Phases 7 and 8 are single-repo and orthogonal to the core bridge surface.
