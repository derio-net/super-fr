...
- **Phase 8 — Agent dispatch prompt: pre-flight repo sync preamble** (added 2026-05-18 after the post-Phase-4-merge stale-checkout incident)
  ...
  - **Fix is agent-side, not bridge-side**: prepend an unconditional `_SYNC_PREAMBLE` to the dispatch prompt in `src/vk/bridge/prompt.py`. The preamble fires UNCONDITIONALLY (no deps gate) and tells the agent to run `git fetch origin && git rebase origin/main` before any other work, stopping and reporting if rebase produces conflicts.
  ...
  - Single-task phase. Single-file source change (`src/vk/bridge/prompt.py`), two unit tests (preamble fires unconditionally + ordering: sync before deps before agent-identity), patch version bump.
