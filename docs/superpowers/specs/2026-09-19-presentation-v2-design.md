...
| Recording | Internal **training** repo (AI102 course); **rendered deck is private**; source is generic and in-repo; **no binaries committed, ever** |
...
`agentic-playground` — a **training repo from the internal AI102 course**, not
product code. Java 17 + Maven; a `logic` package with a Gaussian-Mixture-Model
classifier for POS security-scale weights, plus a `refactoring` package of
deliberately awkward classes. Remote is company GitLab; default branch is
`master`.

**This materially relaxes the confidentiality problem.** Course material with a
generic mathematical core is not proprietary product code. What still must not
appear is the host and namespace. The operator has explicitly permitted reusing
parts of its README to construct the `/fr-goal` prompt.

Its README defines four tasks. Recommended issue composition:

| Task | Use | Why |
|---|---|---|
| 2 — fix 3 failing tests in `SparkGMMAcceptorTest` | **Include** | Supplies the failure-and-recovery beat. A corrected run proves more than a clean one. |
| 4 — write unit tests for `Gaussian` (none exist) | **Include** | Gives the Quality angle something real; edge cases are natural TDD material. |
| 3 — refactor `RefactorMain` | **Optional** | Adds a third surface. Include only if sizing allows. |
| 1 — understand `MultivariateNormalMixtureExpectationMaximization` | **Exclude** | Conversational; produces no deliverable and therefore no PR. |
...
- **Repo hygiene** — the GitLab host and namespace are never named in super-fr.
- **Content exposure** — much reduced, but not nil. The classifier's maths is
  generic; the POS security-scale *domain* is mildly identifying. Resolved by
  scope: **the rendered deck is private and given internally only.** `sed` over
  a cast would not have been a reliable alternative anyway — a name can split
  across write boundaries amid escape sequences.
...
`gitlab.local.gebit.de`, and this is the first time the adapter met a real
instance. `list_dir` works and the issues endpoint is reachable, but:
...
- `_hosts.DEFAULT_HOST_BACKENDS` covers only `github.com` / `gitlab.com`, so
  `gitlab.local.gebit.de` resolves to `"github"` unless
