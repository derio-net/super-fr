...
on OpenCode, where `candidates()` (`packages/fr/src/fr/usage/capture.py::candidates`)
...
- `capture()` (`capture.py::capture`): `pairs = candidates(...)`; `if
  ...
  `sessions=()`, dumped as `sessions: []` (`file.py::dump_usage`).
- `require_sessions=True` is passed only from a new host's first
  `fr run resolve` (`commands/run_cmd.py::_capture_on_new_host`, the only such call); the two
  other capture sites, the `deliver` resolve and `fr archive` (event `closeout`,
  `archive.py`), capture with it `False`, which is where the empty list is written.
- The unavailable shape already exists and is already an allowlisted,
  committed shape: `SessionEntry(session=..., unavailable=<reason>)` (`file.py::SessionEntry`),
  ...
  vocabulary is closed (`file.py::_KEPT_REASONS`; `committed_reason`
...
- `capture._merge` (`capture.py::_merge`) carries an earlier capture's entries
...
has done anything would be noise. Only the `deliver` resolve and `fr archive`
(event `closeout`) write the placeholder, plus `fr usage backfill` (§3.5).
...
- **Mixed `fr` versions on one host (o4).** A released `fr` without the new
  `_merge` rule does not recognise the placeholder: if it re-captures a host
  that holds one and now finds sessions, it carries the placeholder forward
  beside the real entries. No dollar error (`run/cost.py::effective_entries`
  counts it as one unavailable). Accepted; §3.2's "never" holds for this `fr`
  and later, and the next capture by this `fr` drops it.
...
4. `fr usage backfill` over an archived run whose cursor names no session and
   keeps no figure writes the same placeholder, never `sessions: []`.
