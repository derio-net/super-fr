...
| 9 | Does `--note` append or replace? | **Append** a dated resolution note to the existing body; never replace it. The original finding text is the audit trail. *(Added 2026-09-18, mid-experiment, after run C asked; answered from the issue's own account of the manual workaround, not from what run A built.)* |
