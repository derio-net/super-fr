...
`aggregates` (spec 2026-09-26-version-bump-churn §3.I) gates everything that
summarises ACROSS rows — the `N rows` figure, the status counts (HTML tiles,
Markdown table) and the "sharp line" panels. The committed renderings pass
False: each aggregate is a second copy of rows already in the tables, so any
two branches that add a row collide in it even when their rows do not. A
committed report is then a pure per-row rendering and conflicts exactly where
`matrix.yaml` does. The ad-hoc report passes True; so do the places a human
reads counts (`fr acceptance summary`/`status`, the digest), which never
render through here.
"""
...
def render(matrix: Matrix, links: LinkBuilder, stamp: str, *, aggregates: bool) -> str:
    ...
    out = [
        ...
        f"from <code>docs/acceptance/matrix.yaml</code>"
        + (f" ({len(rows)} rows)" if aggregates else "")
        + "</p></header>",
    ]
    ...
    if aggregates:
        out.append(
            '<div class="tiles">'
            + "".join(
                f'<div class="tile {c}"><div class="v">{n}</div>'
    ...
    if aggregates:
        ...
        panel(
            "Failing — the workflow is red until these are fixed or re-classified", "failing", "red"
        )
...
def render_markdown(matrix: Matrix, links: LinkBuilder, stamp: str, *, aggregates: bool) -> str:
    ...
    out = [
        ...
        "`docs/acceptance/matrix.yaml`" + (f" ({len(rows)} rows)" if aggregates else "") + "._",
        "",
    ]
    if aggregates:
        out += ["| status | count |", "| --- | ---: |"]
...
def render_deterministic(
    ...
    # No row count in the stamp, and no aggregates below: a committed report
    # is a pure per-row rendering (§3.I).
    stamp = f"links: {link_mode}"
    renderer = render_markdown if fmt == "md" else render
    return renderer(matrix, links, stamp, aggregates=False)
...
def render_report(
    ...
    return render(matrix, links, stamp, aggregates=True)
