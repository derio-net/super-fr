...
4. Append what you learned to the plan journal as you go:
   ...
   Findings carry `--state open|fixed|refuted`; resolve an existing finding with
   `fr journal update --scope plan --slug <plan-slug> --id <id> --state fixed
   --note "resolution"`. This is the durable record the orchestrator reviews
   and the PR body is derived from.
