...
if ! fr_git_resolve; then
  ...
  emit_block 'fr-isolation edit guard unavailable: git could not be resolved, so the isolation context cannot be established. Refusing edits until the dependency is restored.'
  exit 0
fi
