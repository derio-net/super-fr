...
The command operates entirely on the user-supplied ``rework_path`` and does
not consult ``VK_REPO_ROOT`` for any discovery, so these tests do not set
that env var. (Phases 3 / 5 — ``rework`` and ``rework-list`` — do require it,
because they walk ``docs/superpowers/plans/`` and ``archived-plans/``.)
"""
...
def test_rework_add_empty_flag_exits_2(tmp_path: Path, item, source, track) -> None:
    path = _rework_file(tmp_path)
    runner = CliRunner()
    result = runner.invoke(
        app,
...
def test_rework_add_pipe_escape_roundtrip_item(tmp_path: Path) -> None:
    path = _rework_file(tmp_path)
    runner = CliRunner()
    result = runner.invoke(
        app,
...
def test_rework_add_pipe_escape_roundtrip_source(tmp_path: Path) -> None:
    """``--source`` must escape pipes the same way ``--item`` does (spec §6.1)."""
    path = _rework_file(tmp_path)
    runner = CliRunner()
    result = runner.invoke(
        app,
    ...
    assert result.exit_code == 0
    assert r"PR #42 \| thread" in path.read_text()
    assert parse_origin_table(path)[0].source == "PR #42 | thread"
