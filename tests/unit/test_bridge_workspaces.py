"""Unit tests for `fr.bridge.workspaces` — C1, C2, I5.
...
def test_archive_for_card_archives_matching_workspace():  # C1
    ...
    WHEN  fr.bridge.workspaces.archive_for_card(client, simple_id) is called
    THEN  client.update_workspace('ws-1', archived=True) was called
    """
    from fr.bridge.workspaces import archive_for_card
...
def test_reap_orphans_archives_workspaces_with_no_live_card():  # C2
    ...
    WHEN  fr.bridge.workspaces.reap_orphans(client) is called
...
def test_recover_orphan_card_disabled_by_default(monkeypatch, caplog):  # I5
    ...
    WHEN  fr.bridge.workspaces.recover_orphan_card(...) is called
    THEN  no start_workspace call is made
    AND   a warning is logged
    """
    from fr.bridge.workspaces import recover_orphan_card

    monkeypatch.delenv("VK_BRIDGE_RECOVER_ORPHAN_CARDS", raising=False)
    mcp = FakeMcpClient()

    with caplog.at_level(logging.WARNING, logger="fr.bridge.workspaces"):
