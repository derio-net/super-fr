...
class TestBuildIssueBody:
    def test_contains_instruction_section(self) -> None:
        body = _build_issue_body(
            _make_phase(0), Path("/tmp/plan.md"), "org/repo",
    ...
    def test_contains_dependencies_section(self) -> None:
        body = _build_issue_body(
            _make_phase(2), Path("/tmp/plan.md"), "org/repo",
            prev_num=5, total_phases=3, spec="s.md", goal="G.",
        )
    ...
    def test_phase_one_no_prev_issue_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="no prev_num"):
            _build_issue_body(
                _make_phase(1), Path("/tmp/plan.md"), "org/repo",
                prev_num=None, total_phases=3, spec="s.md", goal="G.",
            )

    def test_phase_with_prev_issue(self) -> None:
        body = _build_issue_body(
            _make_phase(3), Path("/tmp/plan.md"), "org/repo",
            prev_num=42, total_phases=4, spec="s.md", goal="G.",
        )
        assert "- Blocked by #42" in body

    def test_header_contains_phase_info(self) -> None:
        body = _build_issue_body(
            _make_phase(1, title="Bootstrap"), Path("/p.md"), "org/repo",
            prev_num=10, total_phases=3, spec="s.md", goal="G.",
        )
        assert "🎯 Phase:  1/3 — Bootstrap [agentic]" in body

    def test_manual_type(self) -> None:
        body = _build_issue_body(
            _make_phase(0, tag="manual"), Path("/p.md"), "org/repo",
            prev_num=None, total_phases=3, spec="s.md", goal="G.",
        )
    ...
    def test_footer_contains_phase_title(self) -> None:
        body = _build_issue_body(
            _make_phase(2, title="Integration"), Path("/p.md"), "org/repo",
            prev_num=10, total_phases=3, spec="s.md", goal="G.",
        )
    ...
    def test_instruction_references_phase_number(self) -> None:
        body = _build_issue_body(
            _make_phase(5, title="Deploy"), Path("/p.md"), "org/repo",
            prev_num=10, total_phases=6, spec="s.md", goal="G.",
        )
...
class TestBuildIssueTitle:
    def test_title_format(self) -> None:
        phase = Phase(
            number=2, title="Content Migration", tag="agentic", tasks=(), tracking_url=None,
        )
