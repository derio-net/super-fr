...
import pytest
from fr.git import add, commit, repo_root, status


class TestRepoRoot:
    def test_returns_path(self) -> None:
        with patch("fr.git._run_git", return_value="/home/user/repo") as mock:
            result = repo_root()
    ...
    def test_strips_trailing_newline(self) -> None:
        with patch("fr.git._run_git", return_value="/home/user/repo\n"):
            result = repo_root()
            assert result == Path("/home/user/repo")
...
class TestAdd:
    def test_add_single_file(self) -> None:
        with patch("fr.git._run_git") as mock:
            add(["src/main.py"])
...
class TestStatus:
    def test_status_returns_output(self) -> None:
        with patch("fr.git._run_git", return_value="M  src/main.py\n") as mock:
            result = status()
